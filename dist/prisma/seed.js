import { PrismaClient, RolUsuario, EstadoPedido, MetodoPago, } from "../generated/prisma/client";
...
async function seedCategorias() {
    const categorias = readJsonFile("categorias.json");
...
async function seedProductos() {
    const productos = readJsonFile("productos.json");
    for (const producto of productos) {
        ...
        if (!categoria) {
            throw new Error(`No se encontró la categoría con id ${producto.categoria_id} para el producto "${producto.nombre}"`);
        }
...
async function seedDetallePedidos() {
    const detalles = readJsonFile("detalle_pedidos.json");
...
async function seedCarritos() {
    const carritos = readJsonFile("carritos.json");
...
async function seedAlertasStock() {
    const alertas = readJsonFile("alertas_stock.json");
