...
import { addToCartSchema, cartItemParamsSchema, updateCartItemQuantityBodySchema, } from "../validators/cart.schemas.js";
const router = Router();
router.use(authenticate);
router.get("/", getCart);
router.post("/items", validate({ body: addToCartSchema }), addToCart);
router.put("/items/:producto_id", validate({
    params: cartItemParamsSchema,
    body: updateCartItemQuantityBodySchema,
}), updateCartItemQuantity);
router.delete("/items/:producto_id", validate({ params: cartItemParamsSchema }), removeCartItem);
