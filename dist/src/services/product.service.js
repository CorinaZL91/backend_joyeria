...
const sumActiveTallasStockLocal = (tallas) => {
...
export const productService = {
    ...
    getProductById: async (id, isAdminView) => {
    ...
    createProduct: async (body, file) => {
        const categoryExists = await productRepository.findCategoryById(body.categoria_id);
        if (!categoryExists) {
            throw new AppError("La categoría no existe", 404);
        }
        const existingProduct = await productRepository.findActiveProductByName(body.nombre);
        if (existingProduct) {
        ...
        try {
            ...
            if (body.usar_tallas === true) {
                ...
                if (tallas.length === 0) {
                    throw new AppError("Debes enviar al menos una talla cuando el producto usa tallas", 400);
                }
    ...
    updateProduct: async (id, body, file) => {
        ...
        if (body.nombre !== undefined) {
            const duplicateProduct = await productRepository.findProductByNameExceptId(body.nombre, id);
            if (duplicateProduct) {
                throw new AppError("Ya existe otro producto con ese nombre", 409);
            }
        }
        const tallasToUse = finalUsarTallas
        ...
        if (finalUsarTallas && (!tallasToUse || tallasToUse.length === 0)) {
            throw new AppError("El producto debe tener al menos una talla activa cuando usa tallas", 400);
        }
        ...
        try {
            ...
            const shouldDeleteTallas = body.tallas !== undefined ||
                (existingProduct.usar_tallas && !finalUsarTallas);
            const updatedProduct = await productRepository.updateProduct(id, updateData, shouldDeleteTallas);
