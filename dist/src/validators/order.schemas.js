...
export const createOrderSchema = z
    ...
    if (data.metodo_pago === "tienda") {
        if (data.direccion_calle !== undefined ||
            data.direccion_ciudad !== undefined ||
            data.direccion_codigo_postal !== undefined) {
            ctx.addIssue({
                code: z.ZodIssueCode.custom,
                message: "No debes enviar dirección cuando el método de pago es en tienda",
                path: ["metodo_pago"],
            });
        }
    }
...
export const updateOrderStatusSchema = z.object({
    estado: z.enum(["pendiente", "en_preparacion", "enviado", "entregado", "cancelado"], {
        message: "Estado inválido. Usa: pendiente, en_preparacion, enviado, entregado o cancelado",
    }),
});
