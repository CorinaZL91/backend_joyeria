import { z } from "zod";
const toBoolean = (defaultValue) => z.preprocess((value) => {
...
const toNumber = (schema) => z.preprocess((value) => {
    if (value === undefined || value === null || value === "") {
        return undefined;
    }
    return Number(value);
}, schema);
const toNumberWithDefault = (defaultValue, schema) => z.preprocess((value) => {
    if (value === undefined || value === null || value === "") {
        return defaultValue;
    }
    return Number(value);
...
const validateDuplicateTallas = (tallas, ctx) => {
    if (!tallas)
        return;
...
const baseProductSchema = z.object({
    ...
    stock: toNumber(z
        ...
        .optional()),
    stock_minimo: toNumberWithDefault(5, z
        .number()
    ...
    if (data.usar_tallas) {
        if (!data.tallas || data.tallas.length === 0) {
            ctx.addIssue({
                code: z.ZodIssueCode.custom,
                path: ["tallas"],
                message: "Debes agregar al menos una talla cuando el producto usa tallas",
            });
        }
        if (data.stock !== undefined) {
            ctx.addIssue({
                code: z.ZodIssueCode.custom,
                path: ["stock"],
                message: "No debes enviar stock general cuando el producto usa tallas",
            });
        }
    }
    if (!data.usar_tallas) {
        if (data.stock === undefined) {
            ctx.addIssue({
                code: z.ZodIssueCode.custom,
                path: ["stock"],
                message: "El stock general es obligatorio cuando el producto no usa tallas",
            });
        }
        if (data.tallas && data.tallas.length > 0) {
            ctx.addIssue({
    ...
    if (data.usar_tallas === true) {
        if (data.tallas !== undefined && data.tallas.length === 0) {
            ctx.addIssue({
                code: z.ZodIssueCode.custom,
                path: ["tallas"],
                message: "El producto debe tener al menos una talla activa cuando usa tallas",
            });
        }
        if (data.stock !== undefined) {
            ctx.addIssue({
...
export const productIdParamsSchema = z.object({
    id: toNumber(z.number().int("El ID debe ser entero").positive("ID de producto inválido")),
});
