...
export const orderRepository = {
  findUserAddressById: async (userId: number) => {
  ...
  findCartItemsForOrder: async (userId: number) => {
  ...
  createOrderFromCart: async (
    ...
    return prisma.$transaction(async (tx) => {
      ...
      for (const item of cartItems) {
        ...
        if (!productoActual) {
          throw new Error(
            `PRODUCT_NOT_FOUND:${item.producto_id}`
          );
        }

        if (!productoActual.activo) {
          throw new Error(
            `PRODUCT_INACTIVE:${productoActual.nombre}`
          );
        }
        ...
        if (productoActual.usar_tallas) {
          if (!item.producto_talla_id) {
            throw new Error(
              `SIZE_REQUIRED:${productoActual.nombre}`
            );
          }
          ...
          if (!tallaActual || !tallaActual.activo) {
            throw new Error(
              `SIZE_UNAVAILABLE:${productoActual.nombre}`
            );
          }

          if (tallaActual.producto_id !== productoActual.id) {
            throw new Error(
              `SIZE_PRODUCT_MISMATCH:${productoActual.nombre}`
            );
          }

          if (tallaActual.stock < item.cantidad) {
            throw new Error(
              `SIZE_STOCK_INSUFFICIENT:${productoActual.nombre}:${tallaActual.talla}`
            );
          }
        } else {
          if (
            ...
            throw new Error(
              `STOCK_INSUFFICIENT:${productoActual.nombre}`
            );
          }
        }
  ...
  findOrderByIdWithDetails: async (id: number) => {
    return prisma.pedido.findUnique({
      where: { id },
      include: {
        detalles: true,
  ...
  findOrderWithRelations: async (id: number) => {
    return prisma.pedido.findUnique({
      where: { id },
  ...
  findOrdersByUserId: async (userId: number) => {
  ...
  cancelOrderAndRestoreStock: async (id: number, detalles: any[]) => {
    return prisma.$transaction(async (tx) => {
