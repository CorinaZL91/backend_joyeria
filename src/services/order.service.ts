...
const handleOrderTransactionError = (error: unknown): never => {
  if (error instanceof Error) {
    const [code, productName, tallaName] = error.message.split(":");

    if (code === "PRODUCT_NOT_FOUND") {
      throw new AppError(
        `El producto con ID ${productName} ya no existe durante el procesamiento del pedido`,
        400
      );
    }
    ...
    if (code === "SIZE_REQUIRED") {
      throw new AppError(
        `Debes seleccionar una talla válida para "${productName}"`,
        400
      );
    }
    ...
    if (code === "SIZE_UNAVAILABLE") {
      throw new AppError(
        `La talla seleccionada para "${productName}" ya no está disponible`,
        400
      );
    }
    ...
    if (code === "SIZE_PRODUCT_MISMATCH") {
      throw new AppError(
        `La talla seleccionada no pertenece al producto "${productName}"`,
        400
      );
    }
    ...
    if (code === "SIZE_STOCK_INSUFFICIENT") {
      throw new AppError(
        `Stock insuficiente para "${productName}" en talla "${tallaName}" durante el procesamiento del pedido`,
        400
      );
    }
    ...
    if (code === "STOCK_INSUFFICIENT") {
      throw new AppError(
        `Stock insuficiente para "${productName}" durante el procesamiento del pedido`,
        400
      );
    }
  }
...
export const orderService = {
  createOrder: async (userId: number, body: CreateOrderInput) => {
    let direccionPedido = {
    ...
    if (body.metodo_pago === MetodoPago.tarjeta) {
      ...
      const calleFinal =
        body.direccion_calle ?? usuario.direccion_calle ?? null;
      const ciudadFinal =
        body.direccion_ciudad ?? usuario.direccion_ciudad ?? null;
      const codigoPostalFinal =
        body.direccion_codigo_postal ?? usuario.direccion_codigo_postal ?? null;
      ...
      const envioDireccionNueva =
        body.direccion_calle !== undefined ||
    ...
    const createdOrderWithRelations =
      await orderRepository.findOrderWithRelations(newOrder.id);
  ...
  getOrderById: async (id: number, userId: number, rol: RolUsuario) => {
  ...
  updateOrderStatus: async (id: number, estado: EstadoPedido) => {
    const order = await orderRepository.findOrderByIdWithDetails(id);
